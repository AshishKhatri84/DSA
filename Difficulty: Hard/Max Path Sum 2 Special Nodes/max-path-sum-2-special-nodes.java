...
class Solution {
    ...
    private int findMaxPathSum(Node node) {
        ...
        // If both children exist, update maxSum and return the max single-path sum
        if (node.left != null && node.right != null) {
            maxSum = Math.max(maxSum, leftSum + rightSum + node.data);
            return Math.max(leftSum, rightSum) + node.data;
        }

        // If only one child exists, return the sum including that child
        return (node.left != null ? leftSum : rightSum) + node.data;
    }

    int maxPathSum(Node root) {
        maxSum = Integer.MIN_VALUE;  // Reset maxSum
        int rootPathSum = findMaxPathSum(root);

        // Handle the case where the root itself might be the max path sum
